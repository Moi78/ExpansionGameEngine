...
void EXP_Camera::TranslateCamera(vec3f trans, bool changeSub) {
	...
	EXP_Component::m_pos = EXP_Component::m_pos + trans;
}

void EXP_Camera::RotateCamera(vec3f rotation) {
	RD_Camera::RotateCamera(rotation);
	//EXP_Component::m_rot = EXP_Component::m_rot + rotation;
}

void EXP_Camera::AddPitch(float pitch) {
	RD_Camera::AddPitch(pitch);
	//EXP_Component::m_rot = EXP_Component::m_rot + vec3f(0.0f, pitch, 0.0f);
}

void EXP_Camera::AddYaw(float yaw) {
	RD_Camera::AddYaw(yaw);
	//EXP_Component::m_rot = EXP_Component::m_rot + vec3f(yaw, 0.0f, 0.0f);

	//EXP_Component::m_rot.DBGPrint();
}

void EXP_Camera::AddRoll(float roll) {
	RD_Camera::AddRoll(roll);
	EXP_Component::m_rot = EXP_Component::m_rot + vec3f(0.0f, 0.0f, roll);
}
