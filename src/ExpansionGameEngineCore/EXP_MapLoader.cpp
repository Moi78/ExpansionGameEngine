...
bool EXP_MapLoader::LoadMap(std::string map, std::string content_path, const bool nocode) {
	if (!std::filesystem::exists(map)) {
...
void EXP_MapLoader::UnloadMap() {
	...
	if (m_levelCode && m_rl) { //If there is a level code object, call it destructor and set it to nullptr
