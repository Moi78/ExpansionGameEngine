...
demoActor::demoActor(EXP_Game* game, vec3f spawn) : EXP_Actor(game, spawn, vec3f(), vec3f(1.0f, 1.0f, 1.0f)) {
	m_cam = new EXP_Camera(m_game, vec3f(0.0f, 0.0f, 1.0f), vec3f(), vec3f(), vec3f(0.0f, 0.0f, 0.0f), 60.0f, 0.01f, 1000.0f);
	LinkComponent(m_cam);

	m_bound = new EXP_RB_Box(m_game, spawn, vec3f(), vec3f(1.0f, 1.0f, 2.0f), 20.0f);

	m_plight = new EXP_PointLight(game, vec3f(0.0f, -1.0f, 0.0f), vec3f(0.0f, 1.0f, 0.0f), 5.0f, 10.0f);
	LinkComponent(m_plight);

	m_sphere = new EXP_StaticMesh(m_game, m_game->GetRenderer()->FetchShaderFromFile("Content/matTest.exmtl"), "/sphere", vec3f(1.0f, 1.0f, 1.0f), vec3f(), vec3f(1.0f, 1.0f, 1.0f));
	LinkComponent(m_sphere);
